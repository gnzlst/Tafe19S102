...
namespace Hotel_Manager
{
    public partial class FoodMenu : MetroForm
    {
        ...
        private void nextButton_Click(object sender, EventArgs e)
        {
            ...
            {
                BreakfastQ = Convert.ToInt32(breakfastQTY.Text);  // add try..catch statement and log.Info to rolling appender
            }
            if (lunchCheckBox.Checked)
            {
                LunchQ = Convert.ToInt32(lunchQTY.Text); // add try..catch statement and log.Info to rolling appender
            }
            if (dinnerCheckBox.Checked)
            {
                DinnerQ = Convert.ToInt32(dinnerQTY.Text); // add try..catch statement and log.Info to rolling appender
            }
